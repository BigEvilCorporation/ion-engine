...
#include "renderer/window/win32/WindowWin32.h"
...
namespace ion
{
	namespace render
	{
		...
		{
			//Still treat pixels as pixels with DPI scaling
			SetProcessDPIAware();
			...
			std::wstring windowClassNameNative = string::StringToNative(windowClassName);
			std::wstring titleNative = string::StringToNative(title);
			...
			windowClass.lpszClassName = windowClassNameNative.c_str();
			...
			//Create window
			m_windowHandle = CreateWindowEx(m_windowStyleEx, windowClassNameNative.c_str(), titleNative.c_str(), m_windowStyle, x, y, m_windowWidth, m_windowHeight, NULL, NULL, m_hInstance, NULL);

			if(!m_windowHandle)
			{
				int err = GetLastError();
				debug::error << "Could not create window, CreateWindowEx() returned " << err << debug::end;
			}
			...
			pixelFormatDesc.dwFlags = PFD_DRAW_TO_BITMAP | PFD_SUPPORT_OPENGL | PFD_SUPPORT_COMPOSITION | PFD_DOUBLEBUFFER;
		...
		{
			SetWindowText(m_windowHandle, string::StringToNative(title).c_str());
		}
