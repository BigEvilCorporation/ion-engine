...
namespace ion
{
	...
	bool ImageFormatBMP::Read(const std::string& filename)
	{
			...
			if (file.Read(&fileHeader, sizeof(FileHeader)))
			{
				...
				if (file.Read(&bmpHeader, sizeof(BMPHeader)))
				{
					...
					if (bmpHeader.imageWidth > 0 || bmpHeader.imageHeight > 0)
					{
						...
						//Seek to data pos
						if (file.Seek(fileHeader.dataOffset, ion::io::SeekMode::Start) == fileHeader.dataOffset)
						{
							//Alloc data buffer
							...
							if (file.Read(imageData.data(), bmpHeader.dataSizeBytes) == bmpHeader.dataSizeBytes)
							{
								...
								if (m_bitsPerPixel >= eIndexed16Colour && m_bitsPerPixel <= eIndexed256Colour)
								{
									...
									if (file.Seek(paletteDataPos, ion::io::SeekMode::Start) == paletteDataPos)
									{
	...
	bool ImageFormatBMP::Write(const std::string& filename)
	{
	...
	int ImageFormatBMP::GetWidth() const
	{
		return m_width;
	}

	int ImageFormatBMP::GetHeight() const
	{
		return m_height;
	}

	void ImageFormatBMP::SetDimensions(int width, int height)
	{
		m_width = width;
		m_height = height;
	...
	int ImageFormatBMP::GetPaletteSize() const
	{
		return m_palette.size();
	}

	u8 ImageFormatBMP::GetColourIndex(int x, int y) const
	{
	...
	void ImageFormatBMP::SetColourIndex(int x, int y, u8 index)
	{
		ion::debug::Assert(x < m_width&& y < m_height, "Out of range");
	...
	ImageFormatBMP::Colour ImageFormatBMP::GetPaletteEntry(int index) const
	{
		ion::debug::Assert(index < m_palette.size(), "Out of range");
	...
	void ImageFormatBMP::SetPaletteEntry(int index, const Colour& colour)
	{
	...
	ImageFormatBMP::Colour ImageFormatBMP::GetPixel(int x, int y) const
	{
		ion::debug::Assert(x < m_width&& y < m_height, "Out of range");
