...
namespace ion
{
	namespace io
	{
		...
		public:
			enum class OpenMode { Read, Write, Append, Edit };

			File();
			File(const std::string& filename, OpenMode openMode = OpenMode::Read);
			virtual ~File();

			virtual bool Open(const std::string& filename, OpenMode openMode = OpenMode::Read);
			virtual bool Open(const std::string& filename, FileDevice* device, OpenMode openMode = OpenMode::Read);
			virtual void Close();
