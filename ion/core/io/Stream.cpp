...
namespace ion
{
	namespace io
	{
		...
		MemoryStream::MemoryStream(const std::vector<u8>& data)
		{
			m_position = 0;
			m_bytes = data;
		}

		MemoryStream::MemoryStream(const void* data, s64 size)
		{
			m_position = 0;
			m_bytes.resize(size);
			memory::MemCopy(m_bytes.data(), data, (u32)size);
		}
		...
		{
			debug::Assert(m_position + size <= GetSize(), "MemoryStream::Read() - Out of range");
			size = std::min(size, GetSize() - m_position);
			memory::MemCopy(data, &m_bytes[m_position], (u32)size);
			m_position += size;
			return size;
		}
		...
		s64 MemoryStream::Write(const void* data, s64 size)
		{
			if(size > 0)
			{
				...
				memory::MemCopy(&m_bytes[m_position], data, (u32)size);
				m_position += size;
			}
