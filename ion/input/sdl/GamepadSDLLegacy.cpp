...
namespace ion
{
	namespace input
	{
		GamepadSDLLegacy::GamepadSDLLegacy(int index, SDL_Joystick* sdlJoystick)
		{
			m_index = index;
		...
		GamepadSDLLegacy::~GamepadSDLLegacy()
		{
		...
		bool GamepadSDLLegacy::IsConnected() const
		{
			return m_connected;
		}

		int GamepadSDLLegacy::ToPlatformButton(GamepadButtons button) const
		{
			...
			switch (button)
			{
				...
				case GamepadButtons::LEFT_SHOULDER:		platformButton = SDL_JOYSTICK_BUTTON_LEFTSHOULDER; break;
				case GamepadButtons::RIGHT_SHOULDER:	platformButton = SDL_JOYSTICK_BUTTON_RIGHTSHOULDER; break;
		...
		ion::Vector2 GamepadSDLLegacy::GetLeftStick() const
		{
		...
		ion::Vector2 GamepadSDLLegacy::GetRightStick() const
		{
		...
		bool GamepadSDLLegacy::CheckButton(GamepadButtons button) const
		{
			int sdlButton = ToPlatformButton(button);
			return (sdlButton < m_numButtons) ? (m_buttons[sdlButton] != 0) : false;
		}

		bool GamepadSDLLegacy::CheckPrevButton(GamepadButtons button) const
		{
			int sdlButton = ToPlatformButton(button);
