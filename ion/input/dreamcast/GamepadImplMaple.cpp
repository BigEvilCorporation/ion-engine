///////////////////////////////////////////////////
// File:		GamepadImplMaple.cpp
// Date:		1st August 2018
// Authors:		Matt Phillips
// Description:	Xbox 360 gamepad handler
///////////////////////////////////////////////////

#include "GamepadImplMaple.h"

namespace ion
{
	namespace input
	{
		GamepadImpl* GamepadImplMaple::FindAvailableController(int index)
		{
			...
			if (index < maple_enum_count())
			{
				if (maple_device_t* device = maple_enum_type(index, MAPLE_FUNC_CONTROLLER))
				{
					...
					{
						impl = new GamepadImplMaple(index);
					}
				}
			}
		...
		GamepadImplMaple::GamepadImplMaple(int index)
		{
			m_index = index;
			m_connected = false;
			memory::MemSet(&m_mapleState, 0, sizeof(cont_state_t));
			memory::MemSet(&m_prevMapleState, 0, sizeof(cont_state_t));
		}

		void GamepadImplMaple::Poll()
		{
		...
		bool GamepadImplMaple::IsConnected() const
		{
			return m_connected;
		}

		int GamepadImplMaple::ToPlatformButton(GamepadButtons button) const
		{
			...
			switch (button)
			{
				...
				case GamepadButtons::LEFT_TRIGGER:		platformButton = CONT_C; break;
				case GamepadButtons::RIGHT_TRIGGER:	platformButton = CONT_D; break;
		...
		ion::Vector2 GamepadImplMaple::GetLeftStick() const
		{
		...
		ion::Vector2 GamepadImplMaple::GetRightStick() const
		{
		...
		bool GamepadImplMaple::CheckButton(GamepadButtons button) const
		{
			int mapleButton = ToPlatformButton(button);
			return (m_mapleState.buttons & mapleButton) != 0;
		}

		bool GamepadImplMaple::CheckPrevButton(GamepadButtons button) const
		{
			int mapleButton = ToPlatformButton(button);
