...
FILE* ImFileOpen(const char* filename, const char* mode)
{
    ...
    {
        fullPath = device->GetFullPath(fullPath);
    }

    return fopen(fullPath.c_str(), mode);
#elif defined(_WIN32) && !defined(IMGUI_DISABLE_WIN32_FUNCTIONS) && !defined(__CYGWIN__) && !defined(__GNUC__)
